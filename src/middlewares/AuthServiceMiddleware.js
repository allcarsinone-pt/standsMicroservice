...
class AuthServiceMiddleware {
    ...
    static async execute(request, response, next) {
        ...
        if(!token) {
            await LogService.execute({from: 'StandsService', data: 'Missing token', date: new Date(), status: 'error'}, logService)
            return response.status(401).json({ error: 'Missing token' })
        }
        const decodedToken = await authService.login(token)
        if(decodedToken.status !== 200) {
            await LogService.execute({from: 'StandsService', data: 'Invalid token', date: new Date(), status: 'error'},logService)
